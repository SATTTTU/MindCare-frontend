                    ...
                    {calendarDays.map((day, index) => (
                        <motion.div
                            ...
                            className={`aspect-square flex items-center justify-center text-sm rounded-lg transition-all duration-200 
                                ${day ? 'cursor-pointer hover:ring-2 hover:ring-indigo-300' : 'opacity-50'} 
                                ${getDayColor(day)} 
                                ${day && entriesByDay[day] ? 'text-white font-bold' : 'text-slate-700'}`}
                        >
                            {day && (
                                <div className="flex flex-col items-center">
                                    <span>{day}</span>
                                    {entriesByDay[day] && (
                                        <span className="text-lg mt-1">{moodConfig[entriesByDay[day].predictedMood]?.emoji}</span>
                                    )}
                                </div>
                            )}
                        </motion.div>
